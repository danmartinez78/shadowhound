...
# Check if go2_omniverse LiDAR config exists
LIDAR_CONFIG="$HOME/workspace/go2_omniverse/Isaac_sim/Unitree/Unitree_L1.json"

if [[ ! -f "$LIDAR_CONFIG" ]]; then
    warn "Unitree L1 config not found: $LIDAR_CONFIG"
    exit 1
fi
