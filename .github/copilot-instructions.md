...
- Author docs inside `/docs` using the required YAML front-matter:
  ...
  ---
  ```
- Use Obsidian wikilinks (e.g., `[[software/ros2_setup]]`) for internal references. The CI workflow converts them to standard Markdown for GitHub Pages and the Wiki.
- Store images and other media in `docs/_assets/` and embed them with standard Markdown syntax (`![](_assets/image.png)`), even though wikilink embeds are permitted for vault usage.

### Rendering on GitHub Surfaces
- The CI pipeline runs `tools/link_convert.py` to normalize links for MkDocs and the Wiki. Avoid absolute GitHub URLs to repository files; relative links stay portable after conversion.
- Do **not** commit the generated `docs_web/`, `wiki/`, or `site/` folders—these are build artifacts.
...
- Wikilinks are synchronized to the GitHub Wiki automatically via `tools/wiki_sync.py`; never push directly to the Wiki.

### Commit Messaging
- Use the `docs(<scope>): <message>` format for documentation commits, e.g., `docs(simulation): add gazebo tuning guide`.

### Examples
- Link example: `[[software/autodoc/_index|Autodoc Index]]` (vault) → `[Autodoc Index](software/autodoc/_index.md)` (public output).
- Page skeleton:
  ```markdown
  ---
  tags: [software, setup]
